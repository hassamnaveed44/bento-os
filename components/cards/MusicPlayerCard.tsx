    ...
    <div className="group/player col-span-1 lg:row-span-2 min-h-[380px] rounded-3xl bg-slate-900/70 border border-slate-800 hover:border-pink-500/50 p-6 backdrop-blur-xl flex flex-col justify-between transition-all duration-500 hover:shadow-2xl hover:shadow-pink-500/10 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-pink-400">Lo-Fi Radio</span>
        </div>
      ...
      {/* Center: Vinyl Record with slide-out sleeve effect on hover */}
      <div className="my-auto py-4 flex flex-col items-center justify-center z-10">
        <div className="relative flex items-center justify-center">
          {/* Vinyl Disc */}
          <div
            className={`w-36 h-36 rounded-full bg-gradient-to-tr from-slate-950 via-slate-900 to-slate-950 border-4 border-slate-800/90 shadow-2xl flex items-center justify-center transition-transform duration-700 ease-out group-hover/player:scale-105 ${
              isPlaying
                ? "animate-[spin_4s_linear_infinite] shadow-pink-500/25"
                : "group-hover/player:rotate-12 group-hover/player:translate-x-2"
            }`}
          >
            {/* Concentric Grooves */}
          ...
          {isPlaying && (
            <div className="absolute -bottom-2 flex items-end gap-1 bg-slate-950/90 backdrop-blur-md px-2.5 py-1 rounded-full border border-pink-500/30">
